...
import { deleteCustomPartById, getCustomPartById, insertCustomPart, listCustomParts, upsertCustomPart } from "../server/db";
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  try {
    ...
    if (req.method === "PUT") {
      ...
      if (!id || !name || !partNumber || !partsTypeId || !category) {
        return res.status(400).json({ error: "Missing required fields." });
      }

      const existing = await getCustomPartById(id);
      const imageInput = typeof body.image === "string" ? body.image : existing?.image ?? "";
      const image = await persistPartImage(id, imageInput, existing?.image ?? "/parts/_missing.svg");
      ...
      await upsertCustomPart(row);
      return res.status(200).json({ part: toClientPart(row) });
    }
